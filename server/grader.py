...
def compute_agentic_recall(
    ...
    # Per-typology source map. Pulled from ground_truth.payload_json so the
    # gold set scales with what the case actually contains (tier-driven).
    typology_sources = {
        "aks_violation": {
            "prescription_drug_events", "general_ledger", "referral_payments",
            "intercepted_comms", "scanned_claims", "evidence_documents",
        },
        "dead_patient_claim": {"beneficiary_summary", "carrier_claims",
                               "scanned_claims", "evidence_documents"},
        "duplicate_bill": {"carrier_claims", "evidence_documents"},
        "upcoding": {"carrier_claims", "scanned_claims", "evidence_documents"},
        "unbundling": {"carrier_claims"},
        "phantom_beneficiary": {"carrier_claims", "beneficiary_summary"},
        "off_label_marketing": {"prescription_drug_events", "carrier_claims"},
        "double_billing": {"government_contracts", "contract_invoices"},
        "cost_pricing_fraud": {"government_contracts"},
        "product_substitution": {"government_contracts", "contract_deliveries"},
        "ppp_fraud": {"loan_applications", "payroll_records"},
        "foreign_affiliation": {"foreign_affiliations", "corporate_registry"},
    }
    kind_to_sources = {
        "entity": {"corporate_registry", "beneficiary_summary"},
        "shell_link": {"corporate_registry"},
    }
    gold: set[str] = set()
    for kind in gt_kinds:
        gold.update(kind_to_sources.get(kind, set()))
    ...
    try:
        ...
        for (raw,) in rows:
            ...
            gold.update(typology_sources.get(kind, set()))
    except Exception:
        pass
